...
import { isClip, phaseKey, toneFor } from "./QueueCard";
    ...
    <AnimatePresence>
      {id && job && (
        ...
        >
          ...
          <motion.div role="dialog" aria-label={job.title} variants={panelVariants(!!reduced)} className="relative w-full max-w-[680px] rounded-xl border border-border bg-elevated shadow-float">
...
function DetailBody({ jobId, onClose }: { jobId: string; onClose: () => void }) {
  ...
  const setOptions = useQueue((s) => s.setOptions);
  ...
  const o = job.options;
  const editable = job.status !== "downloading" && job.status !== "completed";
    ...
    <div className="p-5">
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            ...
            <Button variant="ghost" size="icon-sm" onClick={onClose} aria-label={t("detail.close")} className="-mr-2 -mt-1">
              <X className="h-4 w-4" />
            </Button>
          </div>
          ...
          <button type="button" onClick={() => void openExternal(job.url)} className="mt-2 flex max-w-full items-center gap-1 text-xs text-fg-faint hover:text-accent focus-ring rounded-sm" title={job.url}>
            <span className="truncate">{job.url}</span>
            <ExternalLink className="h-3 w-3 shrink-0" />
          </button>
          {!editable && (
            <div className="mt-3 flex flex-wrap gap-1">
              <Badge variant="outline">{o.mode === "audio" ? `${o.audioFormat.toUpperCase()} ${t("fmt.audio").toLowerCase()}` : `${o.quality === "best" ? t("fmt.best") : `${o.quality}p`} · ${o.container.toUpperCase()}`}</Badge>
              {o.embedThumbnail && <Badge variant="outline">{t("detail.thumbnail")}</Badge>}
              {o.embedMetadata && <Badge variant="outline">{t("detail.metadata")}</Badge>}
              {o.subtitles && (
                <Badge variant="outline">
                  {t("detail.subs")} {o.embedSubtitles ? t("detail.subsEmbedded") : ""}
                </Badge>
              )}
              {clip && <Badge variant="outline">{t("detail.clip", { a: o.clipStart || "0:00", b: o.clipEnd || "∞" })}</Badge>}
            </div>
          )}
        </div>
      </div>

      {/* Editable format for items that haven't started */}
      {editable && (
        <div className="mt-4 rounded-lg border border-border bg-sunken p-3">
          <div className="mb-2 flex items-baseline gap-2">
            <span className="text-xs font-medium">{t("detail.format")}</span>
            <span className="text-2xs text-fg-faint">{t("detail.formatHint")}</span>
          </div>
          <FormatPicker options={o} onChange={(patch) => setOptions(job.id, patch)} compact audioOnly={job.platform === "spotify"} />
        </div>
      )}
      ...
      <div className="mt-5">
        <div className="mb-1.5 flex items-center justify-between text-xs">
          ...
          <span className="num text-fg-muted">{indeterminate ? "" : formatPercent(percent)}</span>
        </div>
        <ProgressBar percent={percent} tone={toneFor(job.status)} height={6} className={cn(indeterminate && "opacity-50")} />
        <div className="num mt-2 grid grid-cols-4 gap-2 text-xs">
          <Stat label={t("detail.speed")} value={busy ? formatSpeed(p?.speed) : "—"} />
          <Stat label={t("detail.eta")} value={busy && p?.eta != null ? formatEta(p.eta) : "—"} />
          <Stat label={t("detail.downloaded")} value={p?.downloaded ? formatBytes(p.downloaded) : "—"} />
          <Stat label={t("detail.size")} value={p?.total ? formatBytes(p.total) : "—"} />
        </div>
        {job.filePath && (
          <div className="mt-3 flex items-center gap-2 rounded-md border border-border bg-sunken px-2.5 py-1.5 text-xs">
            ...
            <Button variant="ghost" size="sm" className="h-6" onClick={() => void api.revealInFolder(job.filePath!)}>
              <FolderOpen className="h-3.5 w-3.5" /> {t("detail.reveal")}
            </Button>
          </div>
        )}
      </div>
          ...
          {job.error.raw && (
            <>
              <button type="button" onClick={() => setShowRaw((v) => !v)} className="mt-2 flex items-center gap-1 text-xs text-fg-faint hover:text-fg focus-ring rounded-sm">
                <ChevronDown className={cn("h-3 w-3 transition-transform", showRaw && "rotate-180")} /> {t("detail.raw")}
              </button>
      ...
      {job.log.length > 0 && (
        <details className="mt-4 group">
          <summary className="cursor-pointer text-xs text-fg-faint hover:text-fg select-none">{t("detail.log", { n: job.log.length })}</summary>
      ...
      <div className="mt-5 flex items-center gap-2">
        {(job.status === "downloading" || job.status === "queued") && (
          <Button variant="secondary" onClick={() => void pause(job.id)}>
            <Pause className="h-3.5 w-3.5" /> {t("detail.pause")}
          </Button>
        )}
        {job.status === "paused" && (
          <Button onClick={() => resume(job.id)}>
            <Play className="h-3.5 w-3.5" /> {t("detail.resume")}
          </Button>
        )}
        {(job.status === "failed" || job.status === "canceled") && (
          <Button onClick={() => retry(job.id)}>
            <RotateCcw className="h-3.5 w-3.5" /> {t("detail.retry")}
          </Button>
        )}
        ...
        >
          <Trash2 className="h-3.5 w-3.5" /> {t("detail.remove")}
        </Button>
      </div>
    </div>
  );
