    ...
    <div className="mt-2 rounded-md border border-border bg-sunken">
      <div className="flex items-center gap-2 border-b border-border px-2.5 py-1.5">
        ...
        <span className="num text-xs text-fg-muted">
          {item.selected.length} of {entries.length} selected
        </span>
        <div className="relative ml-auto w-44">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter" className="h-6 pl-6 text-xs" />
        </div>
      </div>
      <ul className="max-h-56 overflow-y-auto py-1">
        {visible.map((e, i) => {
          ...
          return (
            <li key={e.id}>
              <label
                className={cn(
                  "flex cursor-pointer items-center gap-2.5 px-2.5 py-1 text-[13px] hover:bg-elevated",
                  !on && "text-fg-muted",
                )}
              >
        ...
        {visible.length === 0 && <li className="px-3 py-2 text-xs text-fg-faint">No items match.</li>}
      </ul>
    </div>
  );
}
